// Ported verbatim from legacy/js/data.js (ESE2027 v63). Do not hand-edit the content below; regenerate with scripts/port-data.cjs.
...
export function dLabel(dt){ return MON[dt.getMonth()] + " " + dt.getDate(); }
export function eachDay(startISO,endISO,cb){ let d=new Date(startISO+"T00:00:00"); const end=new Date(endISO+"T00:00:00"); while(d<=end){ cb(new Date(d)); d.setDate(d.getDate()+1);} }
export function studyDay(dt,subject,tag,plan,badge){ return { date:dLabel(dt), day:WD[dt.getDay()], subject, badge, sessions:[
...
export function mockDay(dt,subject,tag,testName,badge){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:subject+" — Mock Day", badge:badge||"MOCK", sessions:[
...
export function examDay(dt,n){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:"APTRANSCO Exam Window — Day "+n, badge:"APTRANSCO EXAM", sessions:[
...
export function restDay(dt,label,note){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:label, badge:"RECOVERY", sessions:[
...
export function revDay(dt,pair,badge){ const a=pair[0], b=pair[1]; return { date:dLabel(dt), day:WD[dt.getDay()], subject:"Revision — "+a+" + "+b, badge, sessions:[
...
export function grandTestDay(dt,name,badge){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:"Grand Test — Mixed", badge:badge||"GRAND TEST", sessions:[
...
export function subjectMockMarathonDay(dt,subj){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:subj+" — Mock Marathon", badge:"MOCK MARATHON", sessions:[
...
export function pyqSprintDay(dt,subj){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:"Final Sprint — "+subj, badge:"PYQ SPRINT", sessions:[
...
export function taperDay(dt,subj){ return { date:dLabel(dt), day:WD[dt.getDay()], subject:"Last 10 Days — "+subj, badge:"TAPER", sessions:[
...
export function subjectBlock(start,end,subject,tag,plans,testName,badge){ const out=[]; let p=0; eachDay(start,end,(dt)=>{ if(dt.getDay()===0) out.push(mockDay(dt,subject,tag,testName,badge)); else { out.push(studyDay(dt,subject,tag,plans[Math.min(p,plans.length-1)],badge)); p++; } }); return out; }
export function rotatingRevision(start,end,rotation,badge,gtName){ const out=[]; let r=0; eachDay(start,end,(dt)=>{ if(dt.getDay()===0) out.push(grandTestDay(dt,gtName,badge==="REVISION"?"GRAND TEST":badge)); else { const pair=[rotation[r%rotation.length],rotation[(r+1)%rotation.length]]; r+=2; out.push(revDay(dt,pair,badge)); } }); return out; }
...
export const SCHED=DATA.concat(GEN);
export function baseSubj(s){ return s.split(" — ")[0].split(" (")[0].trim(); }
export const JUMPS: { i: number; label: string; date: string }[] = []; (function(){ let last=null; SCHED.forEach((d,i)=>{ const b=baseSubj(d.subject); if(b!==last){ JUMPS.push({i,label:b,date:d.date}); last=b; } }); })();

export const RANKER_QUOTES = [
