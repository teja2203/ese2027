...
class FocusTimerService : Service() {
        ...
        when (intent?.action) {
            ACTION_START -> {
                ...
                val remainingSeconds = intent.getIntExtra(EXTRA_REMAINING, -1)
                    .takeIf { it > 0 }
                startFocusSession(duration, strictMode, soundMode, soundVolume, loop, endTimeOverride, remainingSeconds)
            }
    ...
    private fun startFocusSession(
        ...
        // Start the website DNS shield. If the user has not granted VPN consent
        // yet, the session stays usable and the UI reports the real permission.
        if (VpnService.prepare(this) == null) {
            startForegroundService(Intent(this, WebsiteBlockingVpnService::class.java).apply {
    ...
    /** Session lifecycle helpers: keeps BlockingPrefs and both shields in sync. */
    private fun disableBlockingAndShields() {
        BlockingPrefs.setFocusActive(this, false)
        startService(Intent(this, BlockingOverlayService::class.java).apply {
            action = BlockingOverlayService.ACTION_HIDE
        })
        startService(Intent(this, WebsiteBlockingVpnService::class.java).apply {
        ...
        scope.launch {
            try {
                ...
                if (session != null && session.status == "active" && session.endTime > now) {
                    ...
                    if (VpnService.prepare(this@FocusTimerService) == null) {
                        startForegroundService(Intent(this@FocusTimerService, WebsiteBlockingVpnService::class.java).apply {
                            action = WebsiteBlockingVpnService.ACTION_START
                        })
                    }
                    startTimer()
    ...
    private fun showCompletionNotification() {
        val notification = NotificationCompat.Builder(this, NotificationHelper.CHANNEL_FOCUS_COMPLETE)
            ...
            .setContentText("Great work. You've completed your focus session.")
    ...
    private fun playEffect(tone: Int) {
        runCatching {
            val generator = ToneGenerator(AudioManager.STREAM_MUSIC, 75)
