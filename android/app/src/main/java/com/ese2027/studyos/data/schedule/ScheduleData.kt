...
object ScheduleData {
    ...
    fun forDate(date: LocalDate): DaySchedule? {
        if (date.isBefore(LocalDate.of(2026, 7, 8)) || date.isAfter(LocalDate.of(2027, 1, 31))) {
            return null
        }
        return allDays.firstOrNull { it.date == dLabel(date) }
    }
