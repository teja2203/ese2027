...
function autoBackupPayload(){
  ...
  return {version:APP_VERSION, exportedAt:new Date().toISOString(), checked:state.checked, log:state.log,
    pomo:state.pomo, theme:state.theme, achievements:state.achievements, celebratedDays:state.celebratedDays,
    mocks:state.mocks, shaky:state.shaky, ratings:state.ratings, freeze:state.freeze,
    habits:state.habits, habitLog:state.habitLog, restDayBank:state.restDayBank, restedDays:state.restedDays,
    sound:state.sound, notifications:state.notif, blocking:state.block};
}
function queueAutoBackup(){
  ...
  autoBackupTimer=setTimeout(()=>{
    ...
    if(payload&&window.AndroidESE.saveBackup) window.AndroidESE.saveBackup(JSON.stringify(payload));
    if(window.eseRequestCloudSync) window.eseRequestCloudSync();
  },900);
}
function saveJSON(k,v){ try{ localStorage.setItem(k,JSON.stringify(v)); queueAutoBackup(); }catch(e){ console.error('saveJSON failed for key='+k,e); } }
...
blocks:'<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M4.5 4.5l15 15"/></svg>',
profile:'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="3.5"/><path d="M5.5 20c1-3 3.5-5 6.5-5s5.5 2 6.5 5"/></svg>',
};
...
if(typeof startNativeFocusIfAvailable==="function") startNativeFocusIfAvailable();
} }
...
function notifSupported(){
  return !!(window.AndroidESE?.postNotification || window.Capacitor?.isNativePlatform?.() || window.Capacitor?.Plugins?.LocalNotifications || ("Notification" in window));
}
function askNotifPermission(){
  if(!notifSupported()) return Promise.resolve("unsupported");
  if(window.AndroidESE?.requestNotificationPermission){
    const result=window.AndroidESE.requestNotificationPermission();
    if(result!=="pending") return Promise.resolve(result);
    return new Promise(resolve=>setTimeout(()=>resolve(window.AndroidESE.notificationPermission()),1200));
  }
...
function notify(title,body){
  ...
  if(window.AndroidESE?.postNotification){ window.AndroidESE.postNotification(title,body,title.toLowerCase().includes("achievement")?"achievement":title.toLowerCase().includes("day")?"day":"session"); return; }
  if(window.Capacitor?.isNativePlatform?.() || window.Capacitor?.Plugins?.LocalNotifications){
    const LN = window.Capacitor?.Plugins?.LocalNotifications;
    if(LN){
      LN.schedule({
...
function notifOn(){
  if(window.AndroidESE?.notificationPermission) return !!state.notif&&window.AndroidESE.notificationPermission()==="granted";
...
function startNativeFocusIfAvailable(){
if(!window.AndroidESE?.startNativeFocus||nativeFocusStarted) return;
nativeFocusStarted=true;
window.AndroidESE.startNativeFocus(state.pomo.workMins,state.pomo.breakMins,state.pomo.loop,!!state.block.strict,currentSoundMode,soundVolume,Math.round(getRemainingPomo()));
}
...
nativeFocusStarted=false;   /* allow the native timer to re-sync on resume */
playSound("stop");
}else{
...
if(state.notif&&notifSupported()&&(!window.AndroidESE && typeof Notification!=="undefined" && Notification.permission==="default")) askNotifPermission();
}
saveJSON(POMO_KEY,state.pomo); render(); }
...
if(wasRunning&&window.AndroidESE?.stopNativeFocus){ window.AndroidESE.stopNativeFocus(!!state.block.strict); nativeFocusStarted=false; }
if(wasRunning) playSound("stop");
if(hadMins) toast("Stopped — partial time logged"); }
function skipPhase(){ if(window.AndroidESE?.skipNativeFocus) window.AndroidESE.skipNativeFocus(); completePhase(); }
function setPhase(p){ bankProgress(); state.pomo.logged=0; state.pomo.phase=p; state.pomo.running=false; state.pomo.targetTs=null; state.pomo.timeLeft=phaseSecs(); stopPomoInterval(); if(window.AndroidESE?.stopNativeFocus){ window.AndroidESE.stopNativeFocus(false); nativeFocusStarted=false; } saveJSON(POMO_KEY,state.pomo); render(); }
function applyPreset(w,b){ bankProgress(); state.pomo.logged=0; state.pomo.workMins=w; state.pomo.breakMins=b; state.pomo.running=false; state.pomo.targetTs=null; state.pomo.timeLeft=phaseSecs(); stopPomoInterval(); if(window.AndroidESE?.stopNativeFocus){ window.AndroidESE.stopNativeFocus(false); nativeFocusStarted=false; } saveJSON(POMO_KEY,state.pomo); render(); }
...
/* ── navigation ───────────────────────────────────────── */
/* WebView and browser disagree on which scrolling element owns the page.
   Reset all candidates so route/day changes always land at the real top. */
function scrollToTop(behavior){
const se=document.scrollingElement||document.documentElement;
try{ window.scrollTo({top:0,left:0,behavior:behavior||"auto"}); }catch(e){ window.scrollTo(0,0); }
se.scrollTop=0; document.documentElement.scrollTop=0; document.body.scrollTop=0;
}
function navDay(dir){ state.index=Math.max(0,Math.min(SCHED.length-1,state.index+dir)); saveJSON(IDX_KEY,state.index); scrollToTop("smooth"); render(); }
function jumpTo(i){ state.index=Math.max(0,Math.min(SCHED.length-1,i)); saveJSON(IDX_KEY,state.index); scrollToTop("smooth"); render(); }
...
d.innerHTML = `
<div class="td-left">
<button class="nt-icon press td-profile-btn" aria-label="Profile & Settings" title="Profile & Settings">${IC.profile}</button>
...
d.querySelector(".td-profile-btn").onclick = () => setNav("you");
return d;
}
...
/* Native shell: hide the system bars directly — the WebView drops
   document.requestFullscreen() unless the client hosts a custom view,
   and even then the bars don't reliably go away. The fixed overlay
   covers the whole viewport once the bars are gone. */
if(window.AndroidESE?.setFullscreen){ window.AndroidESE.setFullscreen(true); return; }
...
if(window.AndroidESE?.setFullscreen){ window.AndroidESE.setFullscreen(false); return; }
...
/* Android-only focus protection manager. The web shell remains identical,
   while the bridge supplies the real package list, Room persistence, and OS
   permissions on the APK. */
function buildNativeBlockingCard(){
const root=el("div"); root.className="native-block-manager";
if(!window.AndroidESE){
root.innerHTML=`<div style="font-size:12px;color:var(--ink-3);padding:10px 0">App and website blocking is available in the installed Android app.</div>`;
return root;
}
let tab="apps", query="";
function parse(v,f){ try{return JSON.parse(v||"")||f;}catch(e){return f;} }
function enabledApps(){ return parse(window.AndroidESE.getBlockedApps(),[]).filter(x=>x.isEnabled); }
function enabledSites(){ return parse(window.AndroidESE.getBlockedWebsites(),[]).filter(x=>x.isEnabled); }
function renderManager(){
root.innerHTML="";
const access=el("div"); access.style.cssText="padding:12px;border:1px solid var(--line-2);border-radius:10px;background:var(--card-2);margin-bottom:10px";
access.innerHTML=`<div style="font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase">FOCUS PROTECTION</div><div style="font-size:11px;color:var(--ink-3);line-height:1.5;margin-top:4px">Choose apps and domains to lock while a focus timer is running.</div>`;
if(!window.AndroidESE.isAccessibilityEnabled()){
access.insertAdjacentHTML("beforeend",`<button class="btn btn-acc press" id="grantAccess" style="margin-top:10px;width:100%;font-size:10px">GRANT APP ACCESSIBILITY</button>`);
access.querySelector("#grantAccess").onclick=()=>window.AndroidESE.openAccessibilitySettings();
}
root.appendChild(access);
const tabs=el("div"); tabs.style.cssText="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px";
tabs.innerHTML=`<button class="press ${tab==="apps"?"btn-acc":"btn-ghost"}" style="padding:9px;font-size:10px">APPS</button><button class="press ${tab==="sites"?"btn-acc":"btn-ghost"}" style="padding:9px;font-size:10px">WEBSITES</button>`;
tabs.children[0].onclick=()=>{tab="apps";renderManager();}; tabs.children[1].onclick=()=>{tab="sites";renderManager();}; root.appendChild(tabs);
if(tab==="apps") renderApps(); else renderSites();
}
function renderApps(){
const blocked=enabledApps(), all=parse(window.AndroidESE.getInstalledApps(),[]);
const search=el("input"); search.placeholder="FILTER INSTALLED APPS"; search.value=query; search.style.cssText="width:100%;box-sizing:border-box;padding:10px;border:1px solid var(--line-2);border-radius:8px;background:var(--card-2);color:var(--ink);font-size:11px"; root.appendChild(search);
search.oninput=()=>{query=search.value.toLowerCase();renderManager();};
const list=el("div"); list.style.cssText="margin-top:8px;max-height:240px;overflow-y:auto";
all.filter(a=>!query||(a.appName+" "+a.packageName).toLowerCase().includes(query)).slice(0,80).forEach(a=>{
const on=blocked.some(b=>b.packageName===a.packageName); const row=el("div"); row.style.cssText="display:flex;align-items:center;gap:8px;padding:9px 0;border-top:1px solid var(--line)";
row.innerHTML=`<div style="flex:1;min-width:0"><div style="font-size:12px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.appName}</div><div style="font-size:9px;color:var(--ink-4);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.packageName}</div></div><button class="press ${on?"btn-acc":"btn-ghost"}" style="padding:7px 10px;font-size:9px">${on?"BLOCKED":"ADD"}</button>`;
row.querySelector("button").onclick=()=>{ window.AndroidESE.setBlockedApp(a.packageName,a.appName,!on); renderManager(); toast(on?"App removed from block list":"App blocked during focus"); };
list.appendChild(row); }); root.appendChild(list);
}
function renderSites(){
const sites=enabledSites(); const form=el("div"); form.style.cssText="display:flex;gap:6px";
form.innerHTML=`<input id="siteIn" placeholder="e.g. youtube.com" style="flex:1;min-width:0;padding:10px;border:1px solid var(--line-2);border-radius:8px;background:var(--card-2);color:var(--ink);font-size:11px"><button class="btn btn-acc press" id="siteAdd" style="padding:8px 12px;font-size:10px">ADD</button>`;
form.querySelector("#siteAdd").onclick=()=>{const d=form.querySelector("#siteIn").value.trim();if(!d)return;window.AndroidESE.setBlockedWebsite(d,true);form.querySelector("#siteIn").value="";renderManager();toast("Website added to focus lock");}; root.appendChild(form);
const vpn=el("button"); vpn.className="btn btn-ghost press"; vpn.style.cssText="width:100%;margin-top:8px;font-size:10px"; vpn.textContent="ENABLE WEBSITE BLOCKING"; vpn.onclick=()=>{window.AndroidESE.requestWebsiteBlocking();toast("Website protection permission requested");}; root.appendChild(vpn);
const list=el("div"); list.style.cssText="margin-top:8px"; sites.forEach(s=>{const row=el("div");row.style.cssText="display:flex;align-items:center;padding:9px 0;border-top:1px solid var(--line)";row.innerHTML=`<span style="flex:1;font-size:12px;font-weight:700">${s.domain}</span><button class="press btn-ghost" style="padding:6px 10px;font-size:9px">REMOVE</button>`;row.querySelector("button").onclick=()=>{window.AndroidESE.setBlockedWebsite(s.domain,false);renderManager();};list.appendChild(row);}); root.appendChild(list);
}
renderManager(); return root;
}
...
ach.innerHTML=ah;
return ach; }
/* ════════════════ BLOCKS TAB (PRODUCTION-GRADE APP & WEB SHIELD) ════════════════ */
let blocksActiveTab = "apps";
let blocksSearchQuery = "";
let localBlockedAppsMap = null; // packageName -> appName
let localBlockedSitesSet = null; // domain string

function initBlocksLocalCache(){
  if(!window.AndroidESE) return;
  function parse(v, f){ try{ return JSON.parse(v || "") || f; }catch(e){ return f; } }
  
  if(localBlockedAppsMap === null){
    localBlockedAppsMap = new Map();
    const rawApps = parse(window.AndroidESE.getBlockedApps(), []);
    rawApps.forEach(a => {
      if(a.isEnabled !== false && a.packageName){
        localBlockedAppsMap.set(a.packageName, a.appName || a.packageName);
      }
    });
  }

  if(localBlockedSitesSet === null){
    localBlockedSitesSet = new Set();
    const rawSites = parse(window.AndroidESE.getBlockedWebsites(), []);
    rawSites.forEach(s => {
      if(s.isEnabled !== false && s.domain){
        localBlockedSitesSet.add(s.domain.toLowerCase().trim());
      }
    });
  }
}
...
function renderBlocks(){
  ...
  const inner = el("div"); inner.className = "stagger";
  inner.appendChild(topDeck());
  inner.appendChild(header("Blocks & Focus Protection", "App Lock & Always-On Website Shield"));

  const root = el("div"); root.className = "card"; root.style.cssText = "padding:16px;margin-bottom:20px;";

  if(!window.AndroidESE){
    root.innerHTML = `<div style="font-size:13px;color:var(--ink-3);padding:24px;text-align:center;line-height:1.6;">App and website blocking requires the native Android environment.<br><span style="font-size:11px;color:var(--ink-4);">Please open the installed ESE2027 Android application.</span></div>`;
    inner.appendChild(root);
    wrap.appendChild(inner);
    return wrap;
  }

  initBlocksLocalCache();
  ...
  function renderBlocksContent(){
    ...
    let status = {};
    if(window.AndroidESE.getBlockingStatus){
      try{ status = JSON.parse(window.AndroidESE.getBlockingStatus() || "{}") || {}; }catch(e){ status = {}; }
    }
    const accessOk = status.accessibilityEnabled !== undefined ? status.accessibilityEnabled : (window.AndroidESE.isAccessibilityEnabled ? window.AndroidESE.isAccessibilityEnabled() : false);
    const overlayOk = status.overlayAllowed !== undefined ? status.overlayAllowed : (window.AndroidESE.canDrawOverlays ? window.AndroidESE.canDrawOverlays() : true);
    const vpnOk = status.vpnRunning !== undefined ? status.vpnRunning : (window.AndroidESE.isVpnRunning ? window.AndroidESE.isVpnRunning() : false);
    const currentMode = status.mode !== undefined ? status.mode : (window.AndroidESE.getBlockingMode ? window.AndroidESE.getBlockingMode() : "focus");
    const blockedAppsCount = status.blockedApps !== undefined ? status.blockedApps : (localBlockedAppsMap ? localBlockedAppsMap.size : 0);
    const blockedSitesCount = status.blockedSites !== undefined ? status.blockedSites : (localBlockedSitesSet ? localBlockedSitesSet.size : 0);
    const appLockReady = accessOk && overlayOk;
    const appLockLabel = !accessOk ? "⚠ PERMISSION NEEDED" : (!overlayOk ? "⚠ OVERLAY NEEDED" : (currentMode === "always" ? "● ALWAYS LOCKED" : "● IN STUDY SESSIONS"));

    // Top Status Overview Banner
    const banner = el("div");
    banner.style.cssText = "padding:14px;border:1px solid var(--line-2);border-radius:14px;background:var(--card-2);margin-bottom:16px;";
    banner.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <span style="font-size:10.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--acc);">SYSTEM PROTECTION STATUS</span>
        <span style="font-size:10px;font-weight:700;color:var(--ink-3);">${blockedAppsCount} Apps · ${blockedSitesCount} Sites Shielded</span>
      </div>
      <div style="display:flex;gap:10px;margin-top:12px;">
        <div style="flex:1;padding:10px 12px;background:var(--surface-2);border-radius:10px;border:1px solid ${appLockReady ? 'var(--line-2)' : 'var(--acc)'};">
          <div style="font-size:9.5px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-3);">APP LOCK SERVICE</div>
          <div style="font-size:12px;font-weight:800;margin-top:4px;color:var(--acc);">${appLockLabel}</div>
        </div>
        <div style="flex:1;padding:10px 12px;background:var(--surface-2);border-radius:10px;border:1px solid ${vpnOk ? 'var(--acc)' : 'var(--line-2)'};">
          <div style="font-size:9.5px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-3);">WEBSITE VPN SHIELD</div>
          <div style="font-size:12px;font-weight:800;margin-top:4px;color:${vpnOk ? 'var(--acc)' : 'var(--ink-4)'};">${vpnOk ? '● ALWAYS ON' : '○ DISCONNECTED'}</div>
        </div>
      </div>
      <div style="margin-top:12px;padding-top:10px;border-top:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;">
        <span style="font-size:10.5px;font-weight:700;color:var(--ink-3);">App Locking Rule:</span>
        <div style="display:flex;gap:6px;">
          <button id="modeFocus" class="press ${currentMode === "focus" ? 'btn-acc' : 'btn-ghost'}" style="padding:5px 10px;font-size:10px;font-weight:800;border-radius:6px;cursor:pointer;">
            🎯 IN SESSIONS
          </button>
          <button id="modeAlways" class="press ${currentMode === "always" ? 'btn-acc' : 'btn-ghost'}" style="padding:5px 10px;font-size:10px;font-weight:800;border-radius:6px;cursor:pointer;">
            🔒 ALWAYS LOCKED
          </button>
        </div>
      </div>
    ...
    const modeFocusBtn = banner.querySelector("#modeFocus");
    if(modeFocusBtn){
      modeFocusBtn.onclick = () => {
        if(window.AndroidESE.setBlockingMode) window.AndroidESE.setBlockingMode("focus");
        toast("App lock enforced during study sessions");
        renderBlocksContent();
      };
    }
    const modeAlwaysBtn = banner.querySelector("#modeAlways");
    if(modeAlwaysBtn){
      modeAlwaysBtn.onclick = () => {
        if(window.AndroidESE.setBlockingMode) window.AndroidESE.setBlockingMode("always");
        toast("App lock enforced 24/7 continuously");
        renderBlocksContent();
      };
    }

    if(!accessOk){
      const grantBtn = el("button"); grantBtn.className = "btn btn-acc press";
      grantBtn.style.cssText = "margin-top:12px;width:100%;font-size:10.5px;font-weight:800;letter-spacing:.06em;padding:11px;border-radius:10px;";
      grantBtn.textContent = "GRANT ACCESSIBILITY PERMISSION FOR APP LOCK";
      grantBtn.onclick = () => { window.AndroidESE.openAccessibilitySettings(); };
      banner.appendChild(grantBtn);
    }
    if(accessOk && !overlayOk){
      const overlayBtn = el("button"); overlayBtn.className = "btn btn-acc press";
      overlayBtn.style.cssText = "margin-top:12px;width:100%;font-size:10.5px;font-weight:800;letter-spacing:.06em;padding:11px;border-radius:10px;";
      overlayBtn.textContent = "ALLOW DISPLAY OVER OTHER APPS (FOCUS LOCK)";
      overlayBtn.onclick = () => { window.AndroidESE.openOverlaySettings(); };
      banner.appendChild(overlayBtn);
    }
    ...
    // Sub Navigation Tabs
    const tabs = el("div");
    tabs.style.cssText = "display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px;";
    tabs.innerHTML = `
      <button id="tabBlockedApps" class="press ${blocksActiveTab === "apps" ? "btn-acc" : "btn-ghost"}" style="padding:11px;font-size:11px;font-weight:800;letter-spacing:.06em;border-radius:10px;cursor:pointer;">
        📱 BLOCKED APPS (${blockedAppsCount})
      </button>
      <button id="tabBlockedSites" class="press ${blocksActiveTab === "sites" ? "btn-acc" : "btn-ghost"}" style="padding:11px;font-size:11px;font-weight:800;letter-spacing:.06em;border-radius:10px;cursor:pointer;">
        🌐 WEBSITES (${blockedSitesCount})
      </button>
    `;

    window.eseSetBlocksTab = (tab) => {
      blocksActiveTab = tab;
      renderBlocksContent();
    };

    const tabApps = tabs.querySelector("#tabBlockedApps");
    if(tabApps){
      tabApps.onclick = (e) => { e.preventDefault(); window.eseSetBlocksTab("apps"); };
    }
    const tabSites = tabs.querySelector("#tabBlockedSites");
    if(tabSites){
      tabSites.onclick = (e) => { e.preventDefault(); window.eseSetBlocksTab("sites"); };
    }
    ...
    // TAB 1: APPS
    // ──────────────────────────────────────────
    if(blocksActiveTab === "apps"){
      const allApps = parse(window.AndroidESE.getInstalledApps(), []);

      // Search Box
      const searchBox = el("div");
      searchBox.style.cssText = "position:relative;margin-bottom:14px;";
      searchBox.innerHTML = `
        <input id="blocksAppSearch" placeholder="Search installed apps (e.g. YouTube, Instagram)..." value="${blocksSearchQuery}"
          style="width:100%;box-sizing:border-box;padding:11px 14px;padding-right:36px;border:1px solid var(--line-2);border-radius:10px;background:var(--card-2);color:var(--ink);font-size:12px;font-weight:600;outline:none;">
        ${blocksSearchQuery ? '<button id="clearSearch" class="press" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--ink-3);font-size:14px;cursor:pointer;padding:4px;">✕</button>' : ''}
      `;
      const searchInput = searchBox.querySelector("#blocksAppSearch");
      searchInput.oninput = (e) => {
        blocksSearchQuery = e.target.value.toLowerCase();
        renderAppList();
      };
      ...
      // Page flow (no inner scroll): the fixed timer dock floats over the bottom
      // of the viewport, and an inner scroll container lets its last rows slide
      // underneath the dock — cropping the list. Letting the page scroll keeps
      // every row clear of the dock (the #app bottom padding already accounts
      // for the dock + nav).
      listContainer.style.cssText = "display:flex;flex-direction:column;gap:8px;";
      root.appendChild(listContainer);

      function toggleAppBlock(pkg, name){
        const isCurrentlyBlocked = localBlockedAppsMap.has(pkg);
        if(isCurrentlyBlocked){
          localBlockedAppsMap.delete(pkg);
          toast(`Unblocked ${name}`);
        } else {
          localBlockedAppsMap.set(pkg, name);
          toast(`✓ Added ${name} to session blocking`);
        }
        // Asynchronous native bridge dispatch
        try {
          window.AndroidESE.setBlockedApp(pkg, name, !isCurrentlyBlocked);
        } catch(e) {
          console.error("Native setBlockedApp error:", e);
        }
        renderBlocksContent();
      }
      ...
      function renderAppList(){
        ...
        allApps.forEach(a => {
          const matches = !blocksSearchQuery || (a.appName + " " + a.packageName).toLowerCase().includes(blocksSearchQuery);
          if(!matches) return;
          if(localBlockedAppsMap.has(a.packageName)){
            blockedList.push(a);
          } else {
            availableList.push(a);
          }
        });

        // 1. PINNED BLOCKED APPS SECTION (HIGHLIGHTED)
        if(blockedList.length > 0){
          const headerBlocked = el("div");
          headerBlocked.style.cssText = "font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--acc);margin-top:4px;margin-bottom:2px;display:flex;align-items:center;gap:6px;";
          headerBlocked.innerHTML = `<span>🔒 CURRENTLY BLOCKED (${blockedList.length})</span><div style="flex:1;height:1px;background:var(--acc);opacity:0.3;"></div>`;
          listContainer.appendChild(headerBlocked);

          blockedList.forEach(a => {
            const row = el("div");
            row.className = "press";
            row.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:10px;background:var(--acc-dim);border:1.5px solid var(--acc);box-shadow:0 0 10px rgba(215,25,33,0.12);transition:all .2s ease;cursor:pointer;";
            row.innerHTML = `
              <div style="flex:1;min-width:0;padding-right:12px;">
                <div style="font-size:13px;font-weight:800;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:flex;align-items:center;gap:6px;">
                  <span>${a.appName}</span>
                  <span style="font-size:9px;padding:2px 6px;border-radius:4px;background:var(--acc);color:#fff;font-weight:800;letter-spacing:.04em;">LOCKED</span>
                </div>
                <div style="font-size:10px;color:var(--ink-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">${a.packageName}</div>
              </div>
              <button class="btn btn-acc press" style="padding:7px 14px;font-size:10px;font-weight:800;letter-spacing:.04em;border-radius:8px;cursor:pointer;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.2);">
                ADDED ✓
              </button>
            `;
            row.onclick = () => toggleAppBlock(a.packageName, a.appName);
            listContainer.appendChild(row);
          });
        }

        // 2. AVAILABLE APPS SECTION
        if(availableList.length > 0){
          if(blockedList.length > 0){
            const headerAvail = el("div");
            headerAvail.style.cssText = "font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--ink-3);margin-top:10px;margin-bottom:2px;display:flex;align-items:center;gap:6px;";
            headerAvail.innerHTML = `<span>AVAILABLE APPS (${availableList.length})</span><div style="flex:1;height:1px;background:var(--line);"></div>`;
            listContainer.appendChild(headerAvail);
          }

          availableList.slice(0, 100).forEach(a => {
            const row = el("div");
            row.className = "press";
            row.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:10px;background:var(--surface-2);border:1px solid var(--line);transition:all .15s ease;cursor:pointer;";
            row.innerHTML = `
              <div style="flex:1;min-width:0;padding-right:12px;">
                <div style="font-size:13px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${a.appName}</div>
                <div style="font-size:10px;color:var(--ink-4);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">${a.packageName}</div>
              </div>
              <button class="btn btn-ghost press" style="padding:7px 14px;font-size:10px;font-weight:700;letter-spacing:.04em;border-radius:8px;cursor:pointer;white-space:nowrap;">
                + ADD
              </button>
            `;
            row.onclick = () => toggleAppBlock(a.packageName, a.appName);
        ...
        if(blockedList.length === 0 && availableList.length === 0){
          listContainer.innerHTML = `<div style="font-size:12px;color:var(--ink-3);padding:24px;text-align:center;">No matching applications found.</div>`;
        }
      }
    ...
    // ──────────────────────────────────────────
      ...
      presetContainer.style.cssText = "margin-bottom:14px;";
      presetContainer.innerHTML = `
        <div style="font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--ink-3);margin-bottom:8px;">QUICK SHIELD PRESETS</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px;">
          ${presets.map(p => {
            const isAdded = localBlockedSitesSet.has(p.domain);
            return `<button class="press dpreset-chip ${isAdded ? 'active' : ''}" data-domain="${p.domain}" style="padding:6px 12px;font-size:11px;font-weight:700;border-radius:8px;cursor:pointer;border:1px solid ${isAdded ? 'var(--acc)' : 'var(--line-2)'};background:${isAdded ? 'var(--acc-dim)' : 'var(--surface-2)'};color:${isAdded ? 'var(--acc)' : 'var(--ink-2)'};">
              ${isAdded ? '✓ ' : '+ '}${p.label}
            </button>`;
          }).join("")}
        </div>
      ...
      presetContainer.querySelectorAll("button").forEach(btn => {
        btn.onclick = () => {
          ...
          if(localBlockedSitesSet.has(dom)){
            localBlockedSitesSet.delete(dom);
            toast(`Unshielded ${dom}`);
            window.AndroidESE.setBlockedWebsite(dom, false);
          } else {
            localBlockedSitesSet.add(dom);
            toast(`✓ Shielded ${dom}`);
            window.AndroidESE.setBlockedWebsite(dom, true);
          }
          renderBlocksContent();
        };
      });

      root.appendChild(presetContainer);

      // Custom Domain Entry Form
      const form = el("div");
      form.style.cssText = "display:flex;gap:8px;margin-bottom:14px;";
      form.innerHTML = `
        <input id="siteInput" placeholder="Enter domain (e.g. tiktok.com, twitch.tv)..."
          style="flex:1;min-width:0;padding:11px 14px;border:1px solid var(--line-2);border-radius:10px;background:var(--card-2);color:var(--ink);font-size:12px;font-weight:600;outline:none;">
        <button class="btn btn-acc press" id="siteAddBtn" style="padding:11px 18px;font-size:11px;font-weight:800;letter-spacing:.04em;border-radius:10px;cursor:pointer;white-space:nowrap;">
          SHIELD DOMAIN
        </button>
      `;

      function addCustomSite(){
        const input = form.querySelector("#siteInput");
        const val = input.value.trim().toLowerCase().replace(/^https?:\/\//, '').replace(/\/.*$/, '');
        if(!val) return;
        localBlockedSitesSet.add(val);
        input.value = "";
        toast(`✓ Shielded ${val}`);
        window.AndroidESE.setBlockedWebsite(val, true);
        renderBlocksContent();
      }

      form.querySelector("#siteAddBtn").onclick = addCustomSite;
      form.querySelector("#siteInput").onkeydown = (e) => { if(e.key === "Enter") addCustomSite(); };
      root.appendChild(form);

      // VPN Toggle Action Card
      const vpnCard = el("div");
      vpnCard.style.cssText = "padding:12px 14px;border-radius:10px;background:var(--surface-2);border:1px solid ${vpnOk ? 'var(--acc)' : 'var(--line-2)'};display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;";
      vpnCard.innerHTML = `
        <div style="flex:1;padding-right:10px;">
          <div style="font-size:12.5px;font-weight:800;color:var(--ink);">Always-On Website VPN Protection</div>
          <div style="font-size:10.5px;color:var(--ink-3);margin-top:2px;">Intercepts DNS queries to block distracting domains continuously.</div>
        </div>
        <button class="btn ${vpnOk ? 'btn-ghost' : 'btn-acc'} press" id="vpnToggleBtn" style="padding:8px 16px;font-size:10.5px;font-weight:800;letter-spacing:.04em;border-radius:8px;cursor:pointer;white-space:nowrap;">
          ${vpnOk ? 'RUNNING ●' : 'START SHIELD'}
        </button>
      `;
      vpnCard.querySelector("#vpnToggleBtn").onclick = () => {
        if(window.AndroidESE.startWebsiteBlocking){
          const res = window.AndroidESE.startWebsiteBlocking();
          toast(res || "VPN requested");
          setTimeout(renderBlocksContent, 1000);
        } else if(window.AndroidESE.requestWebsiteBlocking){
          window.AndroidESE.requestWebsiteBlocking();
          toast("Website protection requested");
        }
      };
      root.appendChild(vpnCard);

      // Active Website Shield List
      const siteList = el("div");
      siteList.style.cssText = "display:flex;flex-direction:column;gap:6px;";

      if(localBlockedSitesSet.size === 0){
        siteList.innerHTML = `<div style="font-size:12px;color:var(--ink-3);padding:24px;text-align:center;">No websites currently in shield list.</div>`;
      } else {
        const headerSites = el("div");
        headerSites.style.cssText = "font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--acc);margin-bottom:4px;display:flex;align-items:center;gap:6px;";
        headerSites.innerHTML = `<span>🛡️ ACTIVE SHIELDED DOMAINS (${localBlockedSitesSet.size})</span><div style="flex:1;height:1px;background:var(--acc);opacity:0.3;"></div>`;
        siteList.appendChild(headerSites);

        Array.from(localBlockedSitesSet).sort().forEach(domain => {
          const row = el("div");
          row.style.cssText = "display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-radius:10px;background:var(--acc-dim);border:1.5px solid var(--acc);box-shadow:0 0 8px rgba(215,25,33,0.1);";
          row.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:11px;color:var(--acc);">🛡️</span>
              <span style="font-size:13px;font-weight:800;color:var(--ink);">${domain}</span>
            </div>
            <button class="btn btn-ghost press" style="padding:6px 12px;font-size:10px;font-weight:800;border-radius:6px;cursor:pointer;">
              REMOVE
            </button>
          `;
          row.querySelector("button").onclick = () => {
            localBlockedSitesSet.delete(domain);
...
`App ${APP_VERSION} · permission: <b>${window.AndroidESE?.notificationPermission?window.AndroidESE.notificationPermission():(notifSupported()&&typeof Notification!=="undefined"?Notification.permission:"unsupported")}</b> · pref: ${state.notif?"on":"off"}`,
"ⓘ",()=>{
if(!notifSupported()){ toast("This browser has no Notification API"); return; }
if((window.AndroidESE?.notificationPermission?window.AndroidESE.notificationPermission():Notification.permission)==="granted"){ notify("Test notification","If you can read this, notifications work."); toast("Test sent — did it appear?"); }
else if(!window.AndroidESE && Notification.permission==="denied"){
...
inner.appendChild(acc("blocking",IC.lock,"Blocking & strict mode",state.block.strict?"armed":"",()=>{const body=rows([
...
body.appendChild(buildNativeBlockingCard()); return body; }));
...
row("Automatic backups","Versioned local snapshots are created in the ESE2027 backup folder after every change",window.AndroidESE?.getBackupStatus?(()=>{try{const s=JSON.parse(window.AndroidESE.getBackupStatus());return `${s.count||0} saved`;}catch(e){return "ON";}})():"LOCAL"),
...
else if(state.nav==="blocks") screen=renderBlocks();
...
function renderNav(){
navEl.innerHTML="";
[["today","Today",IC.home],["plan","Plan",IC.plan],["focus","Focus",IC.focus],["progress","Progress",IC.stats],["blocks","Blocks",IC.blocks]].forEach(([id,label,icon])=>{
...
window.addEventListener("online",()=>{ queueAutoBackup(); if(window.eseRequestCloudSync) window.eseRequestCloudSync(); toast("Back online · backup synced"); });
...
window.eseSignIn = function(){
  if(!window.supabase || !window.supabase.createClient){
    toast("Auth library initializing. Please check connection.");
    return;
  }
  form("in");
};
...
p.then(function(r){
  ...
  } else if(mode==="up" && r.data && !r.data.session){
    m.style.color="var(--acc)";
    m.textContent="Account created! Check your email to confirm, then sign in.";
    toast("Confirmation email sent");
  } else if(r.data && r.data.session){
    m.style.color="var(--acc)";
    m.textContent="Signed in successfully!";
    afterLogin(r.data.session);
  }
}).catch(function(err){
  m.style.color="var(--acc)";
  m.textContent=err.message || "Authentication failed";
}); }; }
...
window.eseRequestCloudSync=function(){ if(user) push(); };
function afterLogin(session){
if(!session || !session.user) return;
user=session.user;
toast("Signed in as " + (user.email || "user"));
...
}).catch(function(e){ console.error("afterLogin error", e); hide(); render(); }); }
sb.auth.onAuthStateChange(function(_e,session){ if(session){ afterLogin(session); } else { user=null; render(); } });
sb.auth.getSession().then(function(res){ if(res && res.data && res.data.session){ afterLogin(res.data.session); } });
...
applyTheme();
render();
setTimeout(queueAutoBackup,1200);
